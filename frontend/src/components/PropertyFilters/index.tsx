...
export const PropertyFilters: React.FC<PropertyFiltersProps> = ({ onFiltersChange }) => {
  const [filters, setFilters] = useState<Filters>({})
  const [showAdvancedFilters, setShowAdvancedFilters] = useState(false)
  const [activeFiltersCount, setActiveFiltersCount] = useState(0)
  const [activeFilterTags, setActiveFilterTags] = useState<{key: keyof Filters, label: string, value: string}[]>([])
  ...
  useEffect(() => {
    ...
    if (filters.tipo) {
      ...
      tags.push({ key: 'tipo', label: 'Tipo', value: tipoLabels[filters.tipo] || filters.tipo })
    }
  ...
  return (
    <FiltersContainer>
      <FiltersTitle onClick={toggleAdvancedFilters}>
        Filtros
        {activeFiltersCount > 0 && <FilterCount>{activeFiltersCount}</FilterCount>}
        ...
        <FilterTags>
          {activeFilterTags.map((tag) => (
            <FilterTag key={tag.key}>
              {tag.label}: {tag.value}
              <span onClick={() => handleRemoveFilter(tag.key)}><FaTimes /></span>
            </FilterTag>
          ))}
        </FilterTags>
      )}
      ...
      <AdvancedFiltersContainer isOpen={showAdvancedFilters}>
        <FilterGroup>
          ...
          <FilterSelect
            value={filters.banheiros || ''}
            onChange={(e) => handleInputChange('banheiros', Number(e.target.value))}
          >
            <option value="">Qualquer quantidade</option>
        ...
        <FilterGroup>
          ...
          <PriceRange>
            <FilterInput
              ...
              onChange={(e) => handleInputChange('areaMin', Number(e.target.value))}
            />
            <FilterInput
              type="number"
              placeholder="Área máxima"
              value={filters.areaMax || ''}
              onChange={(e) => handleInputChange('areaMax', Number(e.target.value))}
            />
          </PriceRange>
        </FilterGroup>
      </AdvancedFiltersContainer>
